...
export function computeMeasurement(rows: EventRow[], now = new Date()): MeasurementComputation {
  ...
  const metrics: MetricCard[] = [
    ...
    { key: 'churn_rate', label: 'Churn', displayValue: `${churnRate}%`, rawValue: churnRate, unit: 'percent', deltaPct: churnDelta, direction: direction(churnDelta), isImprovement: churnDelta <= 0, tone: 'pink', comparisonLabel: 'vs prior 30d', spark: churnSpark },
  ];
