...
import {
  ...
  Check, ChevronRight, CircleCheckBig, CircleX, Database, FlaskConical, Gauge,
...
function AnalyticsPage({ data }: Pick<SectionPagesProps, 'data'>) {
  ...
  return <section id="section-analytics" tabIndex={-1} className="section-page command-page analytics-command">
    ...
    <div className="telemetry-layout"><article className="telemetry-canvas command-surface"><div className="surface-kicker"><span><Activity /> {growth.metricLabel} pulse</span><em>{growth.rangeLabel}</em></div><div className="telemetry-value"><span><small>CURRENT VALUE</small><strong>{growth.currentDisplay}</strong></span><b>{measurement?.state === 'measured' ? 'Measured trend' : 'Demo trajectory'}</b></div><div className="telemetry-plot"><div className="telemetry-axis">{growth.axisLabels.map((label) => <span key={label}>{label}</span>)}</div><div className="telemetry-bars">{growth.points.map((point, index) => <i key={point.occurredOn} style={{ height: `${(point.value / growth.axisMax) * 100}%`, '--bar-index': index } as CSSProperties}><b /></i>)}</div><svg viewBox="0 0 100 100" preserveAspectRatio="none"><polyline points={growth.points.map((point, index) => `${index * (100 / Math.max(growth.points.length - 1, 1))},${100 - ((point.value / growth.axisMax) * 100)}`).join(' ')} vectorEffect="non-scaling-stroke" /></svg></div><div className="telemetry-dates">{growth.xAxisLabels.map((label, index) => <span key={`${label}-${index}`}>{label}</span>)}</div></article><aside className="metric-radar command-surface"><div className="surface-kicker"><span><Gauge /> Metric radar</span><em>GOVERNED WINDOW</em></div>{metrics.map((metric, index) => <div className={`radar-metric ${metric.isImprovement ? 'good' : 'bad'}`} key={metric.key}><i>{String(index + 1).padStart(2, '0')}</i><span><small>{metric.label}</small><strong>{metric.displayValue}</strong></span><b>{metric.direction === 'up' ? '↑' : metric.direction === 'down' ? '↓' : '→'} {Math.abs(metric.deltaPct)}%</b></div>)}<div className="radar-note"><Sparkles /><span><b>{measurement?.state === 'measured' ? 'Cohort evidence' : 'Evidence gate'}</b><small>{retentionText}</small></span></div></aside></div>
  </section>;
}
