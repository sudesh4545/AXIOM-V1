...
**Deviation recorded on Day 3 and refined on Day 5.** Persistent dashboard state and the first same-origin event-ingestion path use a managed SQLite-compatible workspace store, so the app can run without a separate process. The PostgreSQL-ready model in `apps/api` is unchanged and remains the scale-up path for heavier analytics. The practical split today is: the workspace store powers the working customer-demo path; FastAPI/PostgreSQL remains the optional analytics-oriented backend path.
...
- Day 3 made dashboard state persistent per operator through a same-origin Next.js route (`apps/web/app/api/v1/dashboard/route.ts`) backed by the managed workspace store via Drizzle. Experiment approval is now a real write: it bumps a snapshot revision, appends the experiment and a Decision Receipt, and records an audit event, all idempotently. The chart and sparkline rendering was rebuilt on SVG polylines after a pixel-versus-percentage unit-mixing bug was identified as the cause of every wrong line in the UI. See `docs/DAY_03_REPORT.md`.
