...
export function AuthScreen({ theme, user, onThemeChange }: AuthScreenProps) {
  ...
  return <main className={`auth-page theme-${theme}`} data-theme={theme}>
    <div className="auth-grid" aria-hidden="true" /><div className="auth-orb auth-orb-one" /><div className="auth-orb auth-orb-two" />
    <header className="auth-topbar"><div className="auth-brand"><i><img src="/brand/axiom-mark-v2-256.png" width="36" height="36" alt="" /></i><strong>A<span>X</span>IOM</strong><em>V1</em></div><div className="auth-theme" role="group" aria-label="Login appearance"><button className={theme === 'light' ? 'active' : ''} onClick={() => onThemeChange('light')} title="Light theme"><Sun /></button><button className={theme === 'dark' ? 'active' : ''} onClick={() => onThemeChange('dark')} title="Dark theme"><Moon /></button><button className={theme === 'neon' ? 'active' : ''} onClick={() => onThemeChange('neon')} title="Neon theme"><Sparkles /></button></div></header>
